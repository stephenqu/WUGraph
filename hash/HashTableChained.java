...
/**
 * HashTableChained implements a hash table with chaining. All objects used must
 * have a valid hashCode() method, which is used to determine which bucket of
 * the hash table an entry is stored in. Each object's hashCode() is presumed to
 * return an int between Integer.MIN_VALUE and Integer.MAX_VALUE. The
 * HashTableChained class implements only the compression function, which maps
 * the hash code to a bucket in the table's range.
 * 
 **/
...
public class HashTableChained {
	 ...
	 * 
	 * @param sizeEstimate
	 *            gives the approximate size of the hash table to make it easier
	 ...
	 * 
	 * @param n
	 *            is the number to compute the next prime
	 ...
	 * 
	 * @param n
	 *            is the number whose primality is to be computed
	...
	int compFunction(int code) {
		...
		int p = computePrime(Math.abs(len * 100)); // In case len is some REALLY
													// large number ie close to
													// 2^31
	...
	/**
	 * Resizes the hash table if the size of this list is greater than
	 * MAX_LOAD*table.length or less than MIN_LOAD*table.length. Does nothing if
	 * size is less than 10, unless table has more than 20 buckets, in which
	 * case table is resized to 19.
	 * 
	 **/
	...
	private void resize() {
		...
		} else if (size > (MAX_LOAD * table.length)
				|| size < (MIN_LOAD * table.length)) {
	...
	/**
	 * Same as above resize function but has a separate signature to make it
	 * faster to find the appropriate size of the hash table
	 * 
	 * @param newSize
	 *            is the new size of the hash table
	 */
